...
//// mise a jour
////
void mise_a_jour(int *ndim_tab, double *T0, double *T1, double *bilan,
                 const double dt) {
...
//// advection
////
void advection(int *ndim_tab, double *T, double *bilan, double *dx, double *a,
               int step) {
...
void diffusion(int *ndim_tab, double *T, double *bilan, double *dx,
               const double mu) {
  for (int64_t j = 2; j < ndim_tab[1] - 2; ++j) {
    for (int64_t i = 2; i < ndim_tab[0] - 2; ++i) {
...
int main(int nargc, char *argv[]) {
  ...
  Ndim_tab[0] =
      (int(dim[0] / nbp_x)) + ((Pos_rankX < dim[0] % nbp_x) ? 1 : 0) + 2 * nfic;
  Ndim_tab[1] =
      (int(dim[1] / nbp_y)) + ((Pos_rankY < dim[1] % nbp_y) ? 1 : 0) + 2 * nfic;
  ...
  int Nitmax = 2000;
  int Stepmax = 2;
  for (int64_t j = 0; j < nfic * 2 * (Ndim_tab[0] + Ndim_tab[1]); ++j) {
    buffer[j] = -40000;
    buffer_s[j] = 40000;
  }
  ...
  for (int64_t nit = 0; nit < Nitmax; ++nit) {
    ...
    for (int64_t step = 0; step < Stepmax; ++step) {
      ...
      // Si on a deux processus
      else if (nbp_y == 2 && nbp_x == 1) {
        // gauche droite
        for (int64_t ific = 0; ific < nfic; ++ific) {
          // periodicité en Imax et Imin
          for (int64_t j = 0; j < Ndim_tab[1]; ++j) {
        ...
        for (int64_t ific = 0; ific < nfic; ++ific) {
          ...
          for (int64_t i = 0; i < Ndim_tab[0]; ++i) {
            ...
            buffer_s[big_buffer_shift + Ndim_tab[0] * (ific + 2) + i] =
                Tout[l1];
          }
        }
        ...
        for (int rac = 2; rac < 4; rac++) {
          ...
          MPI_Irecv(buffer + shift, size, MPI_DOUBLE, rank_donor, etiquette,
                    world, &request[rac]);
        }
        ...
        for (int rac = 2; rac < 4; rac++) {
          ...
          MPI_Send(buffer_s + shift, size, MPI_DOUBLE, rank_dest, etiquette,
                   world);
        }
        ...
        for (int64_t rac = 2; rac < 4; ++rac) {
          int flag = 0;
          MPI_Test(&request[rac], &flag, &stats);
          while (!flag) {
            MPI_Test(&request[rac], &flag, &stats);
          }
        }
      ...
      // Si on a plus de deux processus
      else {
        // YYYYYY
        // YYYYYY
        // YYYYYY
        ...
        for (int rac = 0; rac < 4; rac++) {
          if (rac == 0) {
            if (Pos_rankX == 0) {
              rank_donor = (Pos_rankY + 1) * nbp_x - 1;
            }
            ...
            if (Pos_rankX == nbp_x - 1) {
              rank_donor = nbp_x * Pos_rankY;
            }
